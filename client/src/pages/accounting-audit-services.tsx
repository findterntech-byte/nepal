...
export default function AccountingAuditServicesPage() {
  ...
  const { data, isLoading, refetch } = useQuery<ServiceItem[]>({
    queryKey: ["accounting-audit-services", category],
    queryFn: async () => fetchJson(`/api/accounting-audit-services?${queryParams}`),
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="container mx-auto px-4 py-8">
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold">Accounting & Audit Services</h1>
            <p className="text-sm text-muted-foreground mt-1">Find chartered accountants, tax consultants, and auditors</p>
          </div>
        ...
        <Card className="mb-6">
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6">
            ...
            <div className="space-y-2">
              <Label>Search</Label>
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search firm, specialization..." />
            </div>
              ...
              {filteredData.map((item) => (
                <Card key={item.id} className="hover:shadow-lg transition-all cursor-pointer" onClick={() => setLocation(`/service-details/${item.id}`)}>
                    ...
                    <div className="flex items-start gap-3">
                      <div className="w-12 h-12 rounded-xl bg-amber-100 flex items-center justify-center flex-shrink-0">
                        {item.images && item.images[0] ? (
                          ...
                          <span className="text-amber-600 font-bold text-lg">{item.businessName?.[0] || "A"}</span>
                        )}
                      </div>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    {item.qualification && <div className="text-xs text-muted-foreground">Qualification: {item.qualification}</div>}
