...
import { Plus, Edit, Trash2, Eye, Sprout, X } from "lucide-react";

type AgricultureFormData = {
  title: string;
  description?: string;
  ...
  pesticideApplication?: boolean;
  harvestServices?: boolean;
...
export default function AgricultureSeedsFarmingForm() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AgricultureFormData) => {
      const response = await fetch("/api/admin/agriculture", {
        method: "POST",
    ...
    onSuccess: () => {
      ...
      toast({ title: "Success", description: "Agriculture service created successfully" });
      handleCloseDialog();
    },
    onError: (error: Error) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AgricultureFormData }) => {
      const response = await fetch(`/api/admin/agriculture/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["agriculture"] });
      toast({ title: "Success", description: "Agriculture service updated successfully" });
      handleCloseDialog();
    },
    onError: (error: Error) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["agriculture"] });
      toast({ title: "Success", description: "Agriculture service deleted successfully" });
    },
    onError: (error: Error) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Agriculture, Seeds & Farming</h1>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingService ? "Edit" : "Add"} Agriculture/Seeds/Farming</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="title">Title *</Label>
                <Input id="title" {...register("title", { required: true })} placeholder="Farm/Shop Name" />
              </div>
              <div>
              ...
              <div>
                ...
                <Select onValueChange={(value) => setValue("farmType", value)} defaultValue={editingService?.farmType}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input id="farmSize" {...register("farmSize")} placeholder="e.g. 10 acres" />
              </div>
              <div>
              ...
              <div>
                ...
                <Input id="minimumOrderQuantity" type="number" {...register("minimumOrderQuantity")} placeholder="MOQ" />
              </div>
              <div>
                <Label htmlFor="workingHours">Working Hours</Label>
                <Input id="workingHours" {...register("workingHours")} placeholder="6 AM - 6 PM" />
              </div>
              <div>
                <Label htmlFor="certifications">Certifications</Label>
                <Input id="certifications" {...register("certifications")} placeholder="Organic, FSSAI, etc." />
              </div>
              <div>
                <Label htmlFor="governmentSchemes">Government Schemes</Label>
                <Input id="governmentSchemes" {...register("governmentSchemes")} placeholder="Schemes applied" />
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  "organicFarming", "conventionalFarming", "hydroponics", "aeroponics",
                  "verticalFarming", "greenhouseFarming"
                ].map((item) => (
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  "dairyFarming", "poultryFarming", "fishFarming", "goatFarming", "beeKeeping"
                ].map((item) => (
                  <div key={item} className="flex items-center gap-2">
                    <input type="checkbox" {...register(item as any)} id={item} className="w-4 h-4" />
                    <Label htmlFor={item} className="text-sm font-normal">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  "seeds", "fertilizers", "pesticides", "agriculturalEquipment", "irrigationSystem",
                  "soilTesting", "cropConsultation", "pesticideApplication", "harvestServices"
                ].map((item) => (
                  <div key={item} className="flex items-center gap-2">
                    <input type="checkbox" {...register(item as any)} id={item} className="w-4 h-4" />
                    <Label htmlFor={item} className="text-sm font-normal">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  "dripIrrigation", "sprinklerIrrigation", "solarPower", "farmMachinery",
                  "tractorAvailable", "harvestorAvailable", "coldStorage", "processingUnit", "storageFacility"
                ].map((item) => (
                  <div key={item} className="flex items-center gap-2">
                    <input type="checkbox" {...register(item as any)} id={item} className="w-4 h-4" />
                    <Label htmlFor={item} className="text-sm font-normal">
            ...
            <div className="space-y-3">
              <Label>Certifications & Quality</Label>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  "organicCertified", "fssaiCertified", "govtCertified", "qualityCheck", "labTesting"
                ].map((item) => (
                  <div key={item} className="flex items-center gap-2">
                    <input type="checkbox" {...register(item as any)} id={item} className="w-4 h-4" />
                    <Label htmlFor={item} className="text-sm font-normal">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  "wholesaleAvailable", "retailAvailable", "exportAvailable", "deliveryAvailable",
                  "freeDelivery", "expressDelivery", "packagingAvailable", "customPackaging"
                ].map((item) => (
                  <div key={item} className="flex items-center gap-2">
                    <input type="checkbox" {...register(item as any)} id={item} className="w-4 h-4" />
                    <Label htmlFor={item} className="text-sm font-normal">
            ...
            <div>
              ...
              <Textarea id="description" {...register("description")} placeholder="Describe your products/services..." rows={4} />
            </div>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><strong>Farm Type:</strong> {viewingService?.farmType || "N/A"}</div>
              ...
              <div><strong>Farm Size:</strong> {viewingService?.farmSize || "N/A"}</div>
              <div><strong>Experience:</strong> {viewingService?.experienceYears || "N/A"} years</div>
              <div><strong>Certifications:</strong> {viewingService?.certifications || "N/A"}</div>
            </div>
