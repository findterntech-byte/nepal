...
type GymsFitnessCentersFormData = {
  title: string;
  description?: string;
  facilityType?: string;
  businessName?: string;
  ...
  crossfitArea?: boolean;
  boxingArea?: boolean;
  mmaArea?: boolean;
  yogaArea?: boolean;
  pilatesArea?: boolean;
  spinningArea?: boolean;
  zumbaArea?: boolean;
  aerobicsArea?: boolean;
  steamRoom?: boolean;
  ...
  cricketNets?: boolean;
  footballGround?: boolean;
  tennisCourt?: boolean;
  stretchingArea?: boolean;
  martialArtsArea?: boolean;
  childCareArea?: boolean;
  seniorCitizenArea?: boolean;
...
export default function GymsFitnessCentersForm() {
  ...
  const { register, handleSubmit, reset, setValue } = useForm<GymsFitnessCentersFormData>();
  ...
  const { data: services = [], isLoading } = useQuery({
    queryKey: ["gyms-fitness-centers", activeUserId || null, activeRole || null],
    queryFn: async () => {
      const query = new URLSearchParams();
      if (activeUserId) query.set('userId', activeUserId);
      if (activeRole) query.set('role', activeRole);
      const url = `/api/admin/gyms-fitness-centers${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  const createMutation = useMutation({
    mutationFn: async (data: GymsFitnessCentersFormData) => {
      const response = await fetch("/api/admin/gyms-fitness-centers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      if (!response.ok) { const error = await response.json(); throw new Error(error.message); }
      return response.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["gyms-fitness-centers"] }); toast({ title: "Success", description: "Gym/Fitness Center created successfully" }); handleCloseDialog(); },
    onError: (error: Error) => { toast({ title: "Error", description: error.message, variant: "destructive" }); },
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: GymsFitnessCentersFormData }) => {
      const response = await fetch(`/api/admin/gyms-fitness-centers/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      if (!response.ok) { const error = await response.json(); throw new Error(error.message); }
      return response.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["gyms-fitness-centers"] }); toast({ title: "Success", description: "Updated successfully" }); handleCloseDialog(); },
    onError: (error: Error) => { toast({ title: "Error", description: error.message, variant: "destructive" }); },
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => { const response = await fetch(`/api/admin/gyms-fitness-centers/${id}`, { method: "DELETE" }); if (!response.ok) throw new Error("Failed"); return response.json(); },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["gyms-fitness-centers"] }); toast({ title: "Success", description: "Deleted successfully" }); },
    onError: (error: Error) => { toast({ title: "Error", description: error.message, variant: "destructive" }); },
  });
  ...
  const onSubmit = (data: GymsFitnessCentersFormData) => {
  ...
  const checkboxes = ["wifiAvailable", "parkingAvailable", "changingRooms", "lockersAvailable", "showerAvailable", "towelService", "drinkingWater", "airConditioned", "personalTraining", "groupClasses", "cardioZone", "weightTraining", "freeWeightArea", "machineArea", "functionalTraining", "crossfitArea", "boxingArea", "mmaArea", "yogaArea", "pilatesArea", "spinningArea", "zumbaArea", "aerobicsArea", "steamRoom", "saunaAvailable", "jacuzziAvailable", "swimmingPool", "basketballCourt", "badmintonCourt", "cricketNets", "footballGround", "tennisCourt", "stretchingArea", "martialArtsArea", "childCareArea", "seniorCitizenArea", "nutritionCounseling", "dietPlans", "supplementStore", "cafeteria", "juiceBar", "membershipPlans", "dayPassAvailable", "trialOffer", "corporateMembership"];

  return (
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6"><h1 className="text-2xl font-bold">Gyms & Fitness Centers</h1><Button onClick={() => setIsDialogOpen(true)}><Plus className="mr-2 h-4 w-4" /> Add New</Button></div>
          ...
          <Card key={service.id} className="hover:shadow-md">
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                <p><strong>Type:</strong> {service.facilityType}</p>
                <p><strong>City:</strong> {service.city}</p>
                <p><strong>Area:</strong> {service.totalAreaSqFt} sq ft</p>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editingService ? "Edit Gym/Fitness Center" : "Add Gym/Fitness Center"}</DialogTitle></DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><Label>Facility Type</Label><Input {...register("facilityType")} placeholder="e.g. Gym, Fitness Center" /></div>
              ...
              <div><Label>Opening Time</Label><Input {...register("openingTime")} placeholder="06:00 AM" /></div>
              <div><Label>Closing Time</Label><Input {...register("closingTime")} placeholder="10:00 PM" /></div>
              <div><Label>Number of Trainers</Label><Input type="number" {...register("numberOfTrainers", { valueAsNumber: true })} /></div>
              <div><Label>Number of Equipment</Label><Input type="number" {...register("numberOfEquipment", { valueAsNumber: true })} /></div>
              <div><Label>Total Area (sq ft)</Label><Input type="number" {...register("totalAreaSqFt", { valueAsNumber: true })} /></div>
              <div><Label>Monthly Membership</Label><Input type="number" {...register("monthlyMembership", { valueAsNumber: true })} /></div>
              <div><Label>Yearly Membership</Label><Input type="number" {...register("yearlyMembership", { valueAsNumber: true })} /></div>
