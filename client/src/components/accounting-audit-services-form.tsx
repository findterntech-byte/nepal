...
import { Plus, Edit, Trash2, Eye, Calculator, X } from "lucide-react";

type AccountingAuditFormData = {
  title: string;
  description?: string;
...
export default function AccountingAuditServicesForm() {
  ...
  const { register, handleSubmit, reset, setValue } = useForm<AccountingAuditFormData>();
  ...
  const { data: services = [], isLoading } = useQuery({
    queryKey: ["accounting-audit", userId || null, role || null],
    queryFn: async () => {
      const query = new URLSearchParams();
      if (userId) query.set('userId', userId);
      if (role) query.set('role', role);
      const url = `/api/admin/accounting-audit${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AccountingAuditFormData) => {
      const response = await fetch("/api/admin/accounting-audit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      if (!response.ok) { const error = await response.json(); throw new Error(error.message || "Failed"); }
      return response.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["accounting-audit"] }); toast({ title: "Success", description: "Service created" }); handleCloseDialog(); },
    onError: (error: Error) => toast({ title: "Error", description: error.message, variant: "destructive" }),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AccountingAuditFormData }) => {
      const response = await fetch(`/api/admin/accounting-audit/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      if (!response.ok) { const error = await response.json(); throw new Error(error.message || "Failed"); }
      return response.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["accounting-audit"] }); toast({ title: "Success", description: "Updated" }); handleCloseDialog(); },
    onError: (error: Error) => toast({ title: "Error", description: error.message, variant: "destructive" }),
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const response = await fetch(`/api/admin/accounting-audit/${id}`, { method: "DELETE" });
      if (!response.ok) throw new Error("Failed");
      return response.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["accounting-audit"] }); toast({ title: "Success", description: "Deleted" }); },
    onError: (error: Error) => toast({ title: "Error", description: error.message, variant: "destructive" }),
  });
  ...
  const onSubmit = (data: AccountingAuditFormData) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Accounting & Audit Services</h2>
          <p className="text-muted-foreground">Manage accounting and audit service providers</p>
        </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-2 flex-1">
                    <div className="flex items-start gap-4">
                      <Calculator className="h-5 w-5 text-amber-600 mt-1" />
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editingService ? "Edit" : "Add"} Accounting/Audit Service</DialogTitle></DialogHeader>
            ...
            <Card><CardHeader><CardTitle>Basic Information</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select onValueChange={(v) => setValue("serviceCategory", v)} defaultValue={editingService?.serviceCategory}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="tax_consultant">Tax Consultant</SelectItem>
                        <SelectItem value="bookkeeper">Bookkeeper</SelectItem>
                        <SelectItem value="auditor">Auditor</SelectItem>
                        <SelectItem value="accounting_firm">Accounting Firm</SelectItem>
                        <SelectItem value="tax_filing">Tax Filing Agent</SelectItem>
                        <SelectItem value="gst_return_filing">GST Return Filing</SelectItem>
                  ...
                  <div className="space-y-2"><Label>Qualification</Label><Input {...register("qualification")} placeholder="e.g., CA, CPA, MBA" /></div>
                  <div className="space-y-2"><Label>Specialization</Label><Input {...register("specialization")} placeholder="e.g., Tax, Audit, Advisory" /></div>
                  <div className="space-y-2"><Label>Certifications</Label><Input {...register("certifications")} /></div>
            ...
            <Card><CardHeader><CardTitle>Pricing & Services</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2"><Label>Audit Fee (रू )</Label><Input type="number" {...register("auditFee", { valueAsNumber: true })} /></div>
                  ...
                  <div className="flex items-center space-x-2"><Switch id="taxFiling" onCheckedChange={(checked) => setValue("taxFiling", checked)} /><Label>Tax Filing</Label></div>
                  <div className="flex items-center space-x-2"><Switch id="gstFiling" onCheckedChange={(checked) => setValue("gstFiling", checked)} /><Label>GST Filing</Label></div>
                  <div className="flex items-center space-x-2"><Switch id="bookkeeping" onCheckedChange={(checked) => setValue("bookkeeping", checked)} /><Label>Bookkeeping</Label></div>
                  <div className="flex items-center space-x-2"><Switch id="financialAudit" onCheckedChange={(checked) => setValue("financialAudit", checked)} /><Label>Financial Audit</Label></div>
                  <div className="flex items-center space-x-2"><Switch id="payroll" onCheckedChange={(checked) => setValue("payroll", checked)} /><Label>Payroll Services</Label></div>
                  <div className="flex items-center space-x-2"><Switch id="companyRegistration" onCheckedChange={(checked) => setValue("companyRegistration", checked)} /><Label>Company Registration</Label></div>
      ...
      {viewingService && <Dialog open={!!viewingService} onOpenChange={() => setViewingService(null)}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>{viewingService.title}</DialogTitle></DialogHeader><div className="grid grid-cols-2 gap-4"><div><p className="text-sm font-medium">Business</p><p className="text-sm text-muted-foreground">{viewingService.businessName}</p></div><div><p className="text-sm font-medium">Category</p><p className="text-sm text-muted-foreground">{viewingService.serviceCategory}</p></div><div><p className="text-sm font-medium">User ID</p><p className="text-sm text-muted-foreground">{viewingService.userId || "N/A"}</p></div><div><p className="text-sm font-medium">Role</p><p className="text-sm text-muted-foreground">{viewingService.role || "N/A"}</p></div></div></DialogContent></Dialog>}
    </div>
  );
}